...
export default function ResultsPage() {
  ...
  useEffect(() => {
    if (result) {
      const savedCompletedTips = localStorage.getItem("bmi-completed-tips-" + result.id)
...
  }, [result])
  ...
  const toggleTipCompletion = (tipId: string) => {
    ...
    localStorage.setItem("bmi-completed-tips-" + result!.id, JSON.stringify(Array.from(newCompletedTips)))
  }
